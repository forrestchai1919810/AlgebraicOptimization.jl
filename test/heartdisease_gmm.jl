using LinearAlgebra, Random, Plots, Distributions, CSV, DataFrames, Clustering, Statistics

file_path = "/Users/ForrestChai/Desktop/julia_projects/AlgebraicOptimization.jl/processed.cleveland.data"
...
data_matrix = hcat([Vector{Float64}(data[!, col]) for col in names(data)[1:13]]...)
...
plot_results(x, μ, Σ, z, 4, 5)
savefig("MG_result_hd.png")

assign_vector = [argmax(z[i, :]) for i in 1:N]
ground_truth = Vector{Int}(data[:, 14])
